 ...
 */
function acquia_profile_modules() {
  $modules = array(
 ...
 */
function acquia_profile_details() {
  return array(
    'name' => 'Acquia Drupal',
 ...
 *   task list.
 */
function acquia_profile_task_list() {
 ...
 */
function acquia_profile_tasks(&$task, $url) {
  global $language;
  ...
  // Set the acquia logo
  _acquia_profile_set_logo();

  // Use the Acquia profile when running simpletest.
  variable_set('web_test_case_profile', 'acquia');
...
function acquia_form_alter(&$form, $form_state, $form_id) {
  if ($form_id == 'install_configure') {
    ...
    $form['#submit'] = array('install_configure_form_submit', 'acquia_profile_save_auth');
    
    $form['#validate'] = array('acquia_profile_validate_auth');
...
/**
 * Validate Acquia instance identifier and key.
 */
function acquia_profile_validate_auth($form, &$form_state) {
...
/**
 * Save Acquia instance identifier and key in database for further use.
 */
function acquia_profile_save_auth($form, &$form_state) {
...
function _acquia_profile_set_logo() {
  $settings = variable_get('theme_garland_settings', NULL);
  if ($settings == NULL) {
    $settings = array();
  }
  $settings['logo_path'] = 'profiles/acquia/AcquiaDrupalLogo.png';
